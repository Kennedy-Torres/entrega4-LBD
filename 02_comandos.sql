...
CREATE PROCEDURE AtualizarEstoque(IN pProdutosID INT, IN pNovaQuantidade INT)
BEGIN
    DECLARE precoUnitario DECIMAL(10, 2);
    
    -- Obter o preço unitário do medicamento
    SELECT Preco INTO precoUnitario FROM Produtos WHERE ID = pProdutosID;
    
    -- Atualizar a quantidade do medicamento no estoque
    UPDATE Produtos SET Quantidade = pNovaQuantidade WHERE ID = pProdutosID;
    
    -- Calcular o novo preço total do estoque
    UPDATE Produtos SET Preco = Quantidade * precoUnitario WHERE ID = pProdutosID;
END //
